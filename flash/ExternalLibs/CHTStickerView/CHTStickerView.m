...
- (UIImageView *)noneImageView {
  if (!_noneImageView) {
    _noneImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, defaultInset * 2, defaultInset * 2)];
    _noneImageView.contentMode = UIViewContentModeScaleAspectFit;
    _noneImageView.backgroundColor = [UIColor clearColor];
    _noneImageView.userInteractionEnabled = YES;
  }
  return _noneImageView;
}

- (void)setEnableNone:(BOOL)enableNone {
  _enableNone = enableNone;
  if (self.showEditingHandlers) {
    [self _setEnableClose:enableNone];
  }
}
...
- (id)initWithContentView:(UIView *)contentView {
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    [self setPosition:CHTStickerViewPositionTopRight forHandler:CHTStickerViewHandlerClose];
    [self addSubview:self.closeImageView];
    [self setPosition:CHTStickerViewPositionBottomRight forHandler:CHTStickerViewHandlerRotate];
    [self addSubview:self.rotateImageView];
    [self setPosition:CHTStickerViewPositionTopLeft forHandler:CHTStickerViewHandlerFlip];
...
- (void)setImage:(UIImage *)image forHandler:(CHTStickerViewHandler)handler {
  switch (handler) {
    ...
    case CHTStickerViewHandlerFlip:
      ...
      case CHTStickerViewHandlerNone:
...
- (void)setHandlerSize:(NSInteger)size {
  ...
  [self setPosition:self.flipImageView.tag forHandler:CHTStickerViewHandlerFlip];
    ...
    [self setPosition:self.noneImageView.tag forHandler:CHTStickerViewHandlerNone];
